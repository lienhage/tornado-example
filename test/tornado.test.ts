...
import { genProofArgs, groth16, pedersenHash, rbuffer, toBigIntLE, toBufferLE } from "../utils/circuit";
...
beforeEach(async function () {
  ...
  tornado = await Tornado.deploy(verifier.address, hasher.address, denomination, levels);
});
...
describe("Tornado #withdraw", () => {
  it("should work", async () => {
    ...
    const txResponse = await tornado.deposit(toFixedHex(deposit.commitment), { value: privateTransactionAmount });
    ...
    const nullifierHash = pedersenHash(toBufferLE(deposit.nullifier, 31));
